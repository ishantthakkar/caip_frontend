            ...
            <main className="flex-1 flex flex-col min-h-screen">
                <header className="px-12 py-8 flex justify-between items-center bg-white/80 backdrop-blur-xl border-b border-gray-100 sticky top-0 z-40">
                    <div className="flex items-center gap-5">
                        <div className="w-12 h-12 rounded-2xl bg-[#f0f9f0] flex items-center justify-center text-xl shadow-inner">🏛️</div>
                        <div>
                            <h2 className="text-2xl font-black text-gray-900 tracking-tight">System Master Overview</h2>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">Global Compliance Monitoring</p>
                        </div>
                    </div>
                ...
                <div className="p-12 space-y-12">
                                ...
                                {(() => {
                                    ...
                                    return fullData.map((data, i) => (
                                        <div key={i} className="flex-1 flex flex-col items-center group relative h-full justify-end">
                                            <div className="absolute -top-12 opacity-0 group-hover:opacity-100 transition-all bg-gray-900 text-white text-[10px] font-bold py-1.5 px-3 rounded-lg z-10 shadow-2xl">
                    ...
                    {/* New Row: Latest Defaulters & Industry Chart */}
                                        ...
                                        {stats.recentReports?.length > 0 ? stats.recentReports.map((report: any, i: number) => (
                                            <tr key={report._id} className="hover:bg-gray-50 transition-all font-bold">
                                                ...
                                                <td className="px-6 py-5">
                                                    <p className="text-sm text-gray-900 truncate max-w-[150px]">{report.defaulter_name}</p>
                                            ...
                                            <tr><td colSpan={5} className="py-12 text-center text-gray-400 italic">No recent reports logged</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
                                            ...
                                            <div className="relative w-48 h-48 mb-8">
                                                ...
                                                <div className="absolute inset-0 flex items-center justify-center">
                                                    <div className="text-center">
                                                        ...
                                                        <p className="text-[8px] font-black text-gray-400 uppercase">Sectors</p>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-100">
                                            ...
                                            <th className="px-6 py-4 text-center">Protocol Entries</th>
                                            <th className="px-6 py-4 text-center">Aggregate Debt</th>
                                    ...
                                    <tbody className="divide-y divide-gray-50 font-bold">
                                        {stats.stateInsights?.map((insight: any, i: number) => (
                                            <tr key={i} className="hover:bg-gray-50 transition-all">
                                                <td className="px-6 py-5 text-sm text-gray-900">{insight.state}</td>
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-100">
                                        ...
                                        <th className="px-6 py-4 text-right">Transaction Type</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-50 font-bold">
                                    {(stats.transactions || []).map((tx: any, i: number) => (
                                        <tr key={tx.id} className="hover:bg-gray-50 transition-all">
                                            <td className="px-6 py-5 text-gray-400 text-xs">{i + 1}</td>
                                            <td className="px-6 py-5 text-xs text-gray-600 font-mono italic">{tx.txNo}</td>
                                            <td className="px-6 py-5 text-sm text-gray-900">{tx.member}</td>
                                            ...
                                            <td className="px-6 py-5 text-right">
                                                <span className={`px-4 py-1.5 rounded-full text-[10px] uppercase ${tx.type.includes('New') ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-amber-50 text-amber-600 border border-amber-100'}`}>
                                                    {tx.type}
                                                </span>
                                            </td>
                                        </tr>
                    ...
                    {/* Master Member Registry (Original Table) */}
                    <div className="bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 overflow-hidden">
                            ...
                            <div className="flex gap-4">
                                <div className="relative group">
                                    <input
                                        ...
                                        className="bg-white/10 border border-white/20 rounded-2xl px-12 py-3 text-sm text-white placeholder-white/40 outline-none focus:bg-white focus:text-gray-900 focus:border-white transition-all w-72"
                                    />
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] bg-gray-50/50 border-b border-gray-100">
                                        ...
                                        <th className="px-10 py-6">Identity Verification</th>
                                        ...
                                        <tr key={user._id} className="hover:bg-gray-50 transition-all group">
                                            <td className="px-10 py-6">
                                                <div className="flex items-center gap-5">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-black text-gray-900 truncate max-w-[250px]">{user.name}</p>
                                                        <p className="text-[10px] font-bold text-gray-400 lowercase">{user.email}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-10 py-6">
                                                <p className="text-sm font-bold text-gray-700">{user.district}, {user.state}</p>
                                                <p className="text-[10px] text-gray-400 uppercase font-black mt-1 tracking-widest">{user.subDistrict || 'Global'}</p>
                                            </td>
                                            <td className="px-10 py-6">
                                                <div className="space-y-1 my-1">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[9px] font-black text-gray-300 uppercase">GST:</span>
                                                        <span className="text-xs font-bold text-gray-700 font-mono tracking-tighter">{user.gst}</span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[9px] font-black text-gray-300 uppercase">PAN:</span>
                                                        <span className="text-xs font-bold text-gray-700 font-mono tracking-tighter">{user.pan}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-10 py-6 text-right">
                                                <div className="flex justify-end gap-3 items-center">
                                                    {user.status === '0' ? (
                                                        <button onClick={() => handleAction(user._id, 'approved')} className="px-6 py-2 bg-[#1b5e20] text-white text-[10px] font-black uppercase rounded-xl hover:bg-black transition-all shadow-lg active:scale-95">Verify</button>
                                                    ) : (
                                                        <>
                                                            <button
                ...
                <footer className="bg-white/80 py-8 px-12 border-t border-gray-100 flex justify-between items-center text-gray-400 mt-auto">
                    <p className="text-[11px] font-bold uppercase tracking-[0.3em]">© {new Date().getFullYear()} CAIP Global Oversight Protocol</p>
                    <button onClick={() => { fetchUsers(); fetchAdminStats(); }} className="text-[10px] font-black text-[#1b5e20] bg-[#f0f9f0] px-6 py-2 rounded-full hover:bg-[#1b5e20] hover:text-white transition-all uppercase tracking-widest">
                        Refresh Master Data
                    </button>
                </footer>
            </main>
